...
df <- df |>
  rename(hhincome15 = hhincome2015)
#2. Recode the urbanrural2021 variable to "rural" for 0, "urban" for 1, and "unknown" for 2.
df <- df |>
  mutate(urbanrural2021 = fct_recode(urbanrural2021,
                                     "rural" = "0",
                                     "urban" = "1",
                                     "unknown" = "2"))
#trickier
#3. Recode all the urbanrural variables to "rural" for 0, "urban" for 1, and "unknown" for 2, within the same mutate function.
df <- df |>
  mutate(across(starts_with("urbanrural"), ~ fct_recode(., 
                                                        "rural" = "0",
                                                        "urban" = "1",
                                                        "unknown" = "2")))
#4. Set all values of "unknown" to NA for the urbanrural variables.

df <- df %>%
  mutate(across(starts_with("urbanrural"), ~recode_factor(., "unknown" = NA_character_)))

#### This question was trickier than I meant for it to be! I only showed you how to do this with integer variables, and this is a factor variable. 
#### If you used the below solution to recode to NA, it would have converted the factor levels back to different integers. You could then use the next block of code to go back to factors.

#df <- df |>
#  mutate(across(starts_with("urbanrural"), ~ifelse(. == "unknown", NA, .))) 


#df <- df |>
#  mutate(across(starts_with("urbanrural"), ~ as.factor(.))) |> #convert these variables back to factors
#  mutate(across(starts_with("urbanrural"), ~ fct_recode(., 
#                                                        "rural" = "1",
#                                                        "urban" = "2")))
...
df_networth <- df |>
  select(ID, yob, urbanrural2021, race, hhnetworth_30, hhnetworth_35, hhnetworth_40)

#2. Reshape df_networth so that the net worth variables are in long format.

df_networth_long <- df_networth |>
  pivot_longer(cols = starts_with("hhnetworth"),
               names_to = "atage",
               values_to = "networth")
...
#Option 1
df_networth_long <- df_networth_long |>
  mutate(atage = str_sub(atage, -2)) |>
  mutate(atage = str_c("at", atage))

#Option 2
df_networth_long <- df_networth_long |>
  mutate(atage = str_replace(atage, "hhnetworth_", "at"))


#4. Create a new variable for the year each R was 30.
df_networth <- df_networth |>
  mutate(year_30 = yob + 30)
...
df_networth |> 
  group_by(race, urbanrural2021) |>
  count() |>
  pivot_wider(names_from = urbanrural2021, values_from = n, values_fill = 0) 

ggplot(df_networth, aes(x = race, y = urbanrural2021)) + 
  geom_jitter()

#fancier option that doesn't include the missing values in the visualization.
ggplot(df_networth[complete.cases(df_networth$race, df_networth$urbanrural2021), ], #this is a way to filter out missing values
       aes(x = race, y = urbanrural2021)) + 
  geom_jitter()
...
df_networth |>
  group_by(race) |> #group by parents education, then calculate summary stats per group
  summarize(mean_nw = mean(hhnetworth_40, na.rm = TRUE),
            median_nw = median(hhnetworth_40, na.rm = TRUE),
            sd_nw = sd(hhnetworth_40, na.rm = TRUE),
            min_nw = min(hhnetworth_40, na.rm = TRUE),
            max_nw = max(hhnetworth_40, na.rm = TRUE))
#trickier
...
df_merged <- df_networth |>
  left_join(df_simple, by = "ID") #this will merge the two dataframes on the ID variable, keeping all rows from df_networth and only the rows from df_simple that have a matching ID

#5. Create a scatterplot with household income on the x-axis and household networth on the y-axis. 

ggplot(df_merged, aes(x = hhincome2021, y = hhnetworth_40)) + 
  geom_point() +
